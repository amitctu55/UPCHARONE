<style>
.ListDocument{
    list-style:none;
    font-size:15px;
    padding:4px;
}
.ListDocument li{
    margin:4px;
    font-weight:bold;
}
.fa-check {
    background: white;
    color: #295771;
    border-radius: 23px;
    padding: 4px;
    margin-right: 6px;
}
.img-rounded{
    height: 315px !important;
    width: 100%;
}
</style>

<?php include ("assets/includes/header.php"); ?>
    <?php include ("assets/includes/leftmenu.php"); ?>
        <div class="pag_cstm">

            <div class="row">
                <div class="col-lg-12">
                    <div class="pag_cstm_panel" style="background: #295771;color:white;">
                        <div class="pag_cstm_panel_panel_ontent p-t-0">
                            <div class="row paddb40">
                                	<h4 class="PageTitle">MCI proof</h4>
							<form action='' method='post' enctype="multipart/form-data">
                                <div class="col-sm-12 processsstep3">
                                  
									<p>Dr. <?=$this->session->userdata('drusername');?></p>
                                </div>

                                <div class="col-sm-5 id_proof">

                                    <div class="col-sm-12 bggrya">
									<p>Upload MCI Proof</p>
                                         <div class="img-picker"></div>
										 
                                    </div>
<P>Please upload your identity proof to ensure that the ownership of your profile remains with only you.<br/><b>Acceptable Documents</b></p>

<ul class="ListDocument">

<li><i class="fa fa-check" aria-hidden="true"></i> Medical Registration Certification</li>

<li><i class="fa fa-check" aria-hidden="true"></i> Extra proofs required to be uploaded:</li>




</ul>                                   
                                  <div class="col-sm-8 click_step2 mrt30 padding0">
                                        <a class="backiocn" href="<?=base_url();?>profile_idproof"><i class="fa fa-long-arrow-left" aria-hidden="true"></i>Back</a>
                                    </div>
                                    <div class="col-sm-4 click_step2 mrt20 padding0">
                                        
                                            <button class="continue2"  name='submit' type='submit'>Upload</button>
                                        

                                    </div>

                                </div>
                                
                                <div class="col-sm-1"></div>

                                <div class="col-sm-5 hoslist_he">
                                    
                                    <div class="text-center"><img src="<?=base_url();?>admin1947/public/assets/upload/<?=$src;?>" class="img-responsive img-rounded" alt='No Profile Image'>
									</div>
									
                                    <p>2.5M patients are looking for a doctor on Upchar. Verify your credential and reach out to them</p>
									
                                </div>
								<form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <?php include ("assets/includes/footer.php"); ?>