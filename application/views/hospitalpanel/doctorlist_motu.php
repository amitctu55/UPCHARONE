                ...
                <div class="col-lg-12">
                    <div class="pag_cstm_panel">
					 ...
					 <table id="datatable" class="display" style="width:100%">
								...
								<tbody>
								<?php foreach($doctorlist as $p){ ?>
								<tr><td> <?=$p->fname.' '.$p->lname;?></td>
										<td><?=$p->mobile;?></td>
										<td><?=$d->email;?></td>
										<td><?=($d->p_status)? '<span style="color:#00f;">Approved Active</span>' : '<span style="color:#f00;">Approval Pending</span>';?></td>
										<td>
										<!--<a href='<?=base_url()?>hospitalpanel/updatedoctor/<?=mybase64_encode($p->id);?>'>Update</a>-->
										<a href="<?=base_url();?>hospitalpanel/doctordetail/<?=$p->id;?>" class="view_profile">View Profile</a>
