...
<div class="auth-wrapper">
  <div class="auth-card">
    ...
    <div class="auth-tabs">
      <a href="<?=base_url('medical-login');?>" class="auth-tab-btn">Login</a>
      <a href="<?=base_url('medical-signup');?>" class="auth-tab-btn active">Sign up</a>
    </div>
    ...
    <form class="auth-form" id="medregistrationform" action="<?=base_url();?>Medicaluser/register" method="POST">
      ...
      <div class="form-group">
        ...
        <input type="email" id="email" name="email" class="form-control" placeholder="pharmacy@example.com" required>
      </div>
      ...
      <button type="submit" class="btn-submit" id="sub_button">Register Pharmacy Store</button>
    </form>
...
$(document).ready(function() {
  $('#medregistrationform').submit(function(e) {		
    e.preventDefault();		
    var myform = $(this);	
    var email = $('#email').val();
    ...
    $.ajax({			
      ...
      success: function(response) {			
        ...
        if(response.status === 'success') {					
          ...
          alert(response.msg);
          $('#sub_button').prop('disabled', false).text('Register Pharmacy Store');
        } else {					
