    ...
    <div class="auth-recovery-card">
        ...
        <div id="step1Container">
            <div class="recovery-header">
                ...
                <h2 class="recovery-title">Pathology Lab Recovery</h2>
                <p class="recovery-subtitle">
                    Enter your registered diagnostic lab contact. We'll send a 6-digit verification code to reset your credentials.
                </p>
            </div>

            <form id="pathforgotform" action="<?=base_url('pathlabuser/forgotpass');?>" method="post">
                <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>">
                <div class="form-group-cstm">
                    <label class="form-label-cstm">Registered Mobile / Email ID *</label>
                    <input type="text" name="mobile" class="form-input-cstm" placeholder="e.g. lab@diagnostics.com or 10-digit mobile" required autofocus>
                </div>
        ...
        <div id="step2Container" style="display: none;">
            ...
            <form id="pathforgototpform" action="<?=base_url('pathlabuser/verifyforgototp');?>" method="post">
                <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; font-size: 12.5px;">
                    ...
                    <button type="button" class="resend-link-btn pathresendfotp" id="btnResendOtp">
        ...
        <div id="step3Container" style="display: none;">
            <div class="recovery-header">
                ...
                <p class="recovery-subtitle">
                    Set a strong new password for your Diagnostic Laboratory portal.
                </p>
            </div>

            <form id="pathforgotnewpassform" action="<?=base_url('pathlabuser/setnewpass');?>" method="post">
                <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>">
                <div class="form-group-cstm">
        ...
        <div class="auth-footer-links">
            <a href="<?=base_url('pathlab-login');?>" class="auth-link">
                <i class="fa fa-arrow-left"></i> Back to Pathology Login
            </a>
...
$(document).ready(function() {
    ...
    $('#pathforgotnewpassform').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                try { if (typeof response === 'string') response = JSON.parse(response); } catch(e){}
                if (response && response.status === 'success') {
                    showAlert('Password updated successfully! Redirecting to pathology login...', false);
                    setTimeout(function() {
    ...
    // Resend OTP handler
    $('.pathresendfotp').click(function(e) {
        e.preventDefault();
        var link = $(this);
        ...
        $.ajax({
            type: "POST",
            url: '<?=base_url("pathlabuser/resendforgetotp");?>',
