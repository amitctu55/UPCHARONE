...
class Coursetype extends CI_Controller {
	...
	public function create()
	{
		if(isset($_POST['submit'])){
			...
			if($id=='')
			{
				$count =$this->db->where('course_type_name', $eduname)->count_all_results('master_course_type');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
    				redirect(base_url().'masters/coursetype');
    				exit();
    			}
				if($this->coursetypemodel->coursetypeinsert())
				{
			...
			{
				$count =$this->db->where('course_type_name', $eduname)->where_not_in('course_type_id',$id)->count_all_results('master_course_type');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
				...
				if($this->coursetypemodel->coursetypeedit($id))
				{
	...
	public function statusupdate()
	{
		...
		$this->coursetypemodel->coursetypestatus($uid);
	...
	public function delete()
	{
		...
		if($this->coursetypemodel->coursetypedelete($uid))
		{
			echo "Y";
		}
	}
