...
class Document extends CI_Controller {
	...
	public function create()
	{
		if(isset($_POST['submit'])){
			...
			if($id=='')
			{
				$count =$this->db->where('document_name', $eduname)->count_all_results('master_document');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
    				redirect(base_url().'masters/document');
    				exit();
    			}
				if($this->documentmodel->documentinsert())
				{
			...
			{
				$count =$this->db->where('document_name',$eduname)->where_not_in('document_id',$id)->count_all_results('master_document');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
				...
				if($this->documentmodel->documentedit($id))
				{
	...
	public function statusupdate()
	{
		...
		$this->documentmodel->documentstatus($uid);
	...
	public function delete()
	{
		...
		if($this->documentmodel->documentdelete($uid))
		{
			echo "Y";
		}
	}
