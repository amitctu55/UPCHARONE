...
class Course extends CI_Controller {
	...
	public function create()
	{
		if(isset($_POST['submit'])){
			...
			if($id=='')
			{
				$count =$this->db->where('course_name', $eduname)->count_all_results('master_course');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
    				redirect(base_url().'masters/course');
    				exit();
    			}
				if($this->coursemodel->courseinsert())
				{
			...
			{
				$count =$this->db->where('course_name', $eduname)->where_not_in('course_id',$id)->count_all_results('master_course');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
				...
				if($this->coursemodel->courseedit($id))
				{
	...
	public function statusupdate()
	{
		...
		$this->coursemodel->coursestatus($uid);
	...
	public function delete()
	{
		...
		if($this->coursemodel->coursedelete($uid))
		{
			echo "Y";
		}
	}
