...
class Education extends CI_Controller {
	...
	public function create()
	{
		if(isset($_POST['submit'])){
			...
			if($id=='')
			{
				$count =$this->db->where('education_name', $eduname)->count_all_results('master_education');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
    				redirect(base_url().'masters/education');
    				exit();
    			}
				if($this->educationmodel->educationinsert())
				{
			...
			{
				$count =$this->db->where('education_name',$eduname)->where_not_in('education_id',$id)->count_all_results('master_education');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
				...
				if($this->educationmodel->educationedit($id))
				{
	...
	public function statusupdate()
	{
		...
		$this->educationmodel->educationstatus($uid);
	...
	public function delete()
	{
		...
		if($this->educationmodel->educationdelete($uid))
		{
			echo "Y";
		}
	}
