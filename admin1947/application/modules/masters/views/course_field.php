...
<br>  
  ...
  <div class="row text">
	...
	<form action="<?=base_url()?>masters/coursefield/create" method="post" id="myform">
	<div class="formheader">
		Course Field Master
	</div>
	<div class="formdiv">
		<div class="form-group">
		  <label for="text">Course Field Name:</label>
		  <input type="text" class="form-control" id="coursefield" placeholder="Enter course field" name="coursefield" data-validation="required"
    ...
    <thead>
      <tr>
        <th class="tableheaddata">Course Field ID</th>
        <th class="tableheaddata">Course Field</th>
    ...
    <tbody>
	<?php 
		$getlists=$this->db->get_where('master_course_field');
	...
	?>
      <tr class="active">
        <td class="tabledata"><?=$rowdata->course_field_id;?></td>
        <td class="tabledata"><?=$rowdata->course_field_name;?></td>
		
        <td class="tabledata"><a href="#" class="statuscng " id="ch<?=$rowdata->course_field_id;?>" data-uid="<?=$rowdata->course_field_id;?>" style="cursor:pointer"><span class="<?=$statusclass?>" ><?=$statusvalue?></span></a></td>
        <td class="tabledata" id="kamal"><a href="#" style="cursor:pointer" class="select" data-uid="<?=base64_encode($rowdata->course_field_id)?>" data-name="<?=$rowdata->course_field_name;?>" data-status="<?=$rowdata->status;?>" >Select</a></td>
		
        <td class="tabledata"><a href="#" style="cursor:pointer" class="delete" data-uid="<?=$rowdata->course_field_id;?>">Delete</a></td>
      </tr>
		<?php } ?>
    </tbody>
  </table>
...
$(document).ready(function(){
    ...
    $(".delete").click(function(){
		...
		if(c)
		{
			...
			var uri='<?=base_url()?>masters/coursefield/delete/'+uid
			$.ajax({
			 type:"post", 
			 url: uri,
...
$(document).ready(function(){
	$(".statuscng").click(function(){
		...
		var uri='<?=base_url()?>masters/coursefield/statusupdate';
			$.ajax({
			 type:"post", 
			 url: uri,
			 data:{uid:uid},
