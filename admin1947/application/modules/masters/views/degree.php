<div class="content-wrapper">
  ...
  <section class="content-header" style="padding: 20px 20px 10px;">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
      <div>
        <h1 style="font-size: 22px; font-weight: 700; color: #1e293b; margin: 0;">Degree Master</h1>
        <small style="color: #64748b; font-size: 13px;">Manage medical degrees, qualifications, and certifications</small>
      </div>
      <ol class="breadcrumb" style="position: static; float: none; margin: 0; background: transparent; padding: 0;">
        ...
        <li><a href="<?=base_url('masters/degree')?>" style="color: #64748b;">Masters</a></li>
        <li class="active" style="color: #1e293b; font-weight: 600;">Degree</li>
      </ol>
    </div>
  </section>
      ...
      <div class="row">
          ...
          <div class="master-card master-sticky-form">
            <div class="master-card-header">
              <h3 class="master-card-title">
                ...
                <span id="form-title">Add Degree</span>
              </h3>
            ...
            <form action="<?=base_url('masters/degree/create')?>" method="post" id="master-form" class="master-card-body">
              <input type="hidden" id="eid" name="eid" value="">

              <div class="form-group" style="margin-bottom: 20px;">
                <label for="degree_name" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px;">
                  Degree / Qualification <span style="color: #ef4444;">*</span>
                </label>
                <input type="text" class="form-control" id="degree_name" name="degree" 
                       placeholder="e.g. MBBS, MD, MS, BDS, BAMS" required
                       style="border-radius: 8px; height: 40px;">
                <small class="text-muted" style="font-size: 11.5px; margin-top: 4px; display: block;">Enter recognized medical qualification acronym or title.</small>
              </div>
              ...
              <div style="display: flex; gap: 10px; justify-content: flex-end; align-items: center; border-top: 1px solid #e2e8f0; padding-top: 16px;">
                ...
                <button type="submit" id="submit-btn" name="submit" class="btn btn-primary" style="border-radius: 8px; font-weight: 600; padding: 8px 22px; background: #00a896; border-color: #00a896;">
                  <i class="fa fa-check"></i> <span id="submit-text">Add Degree</span>
                </button>
              </div>
            </form>
          </div>
        ...
        <div class="col-lg-8 col-md-7">
          <div class="master-card">
            <div class="master-card-header">
              <h3 class="master-card-title">
                <i class="fa fa-graduation-cap" style="color: #00a896;"></i>
                <span>Medical Degree List</span>
              </h3>
              <span style="font-size: 12px; color: #64748b; font-weight: 500;">
                Total Records: <strong><?=(isset($total_rows) ? $total_rows : (isset($config['total_rows']) ? $config['total_rows'] : (is_array($degree) ? count($degree) : 0)));?></strong>
              </span>
            </div>

            <div class="master-card-body" style="padding: 16px 20px;">
              <!-- Search & Filters Toolbar -->
              <form action="<?=base_url('masters/degree')?>" method="get" id="search_form" class="master-toolbar" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                ...
                <div style="display: flex; align-items: center; gap: 8px; flex-grow: 1; max-width: 380px; justify-content: flex-end;">
                  <div class="input-group input-group-sm" style="width: 100%;">
                    <input type="text" class="form-control" name="keyword" placeholder="Search by degree name..." 
                  ...
                  <?php if($this->input->get('keyword')!='' || ($this->input->get('status')!==null && $this->input->get('status')!='')): ?>
                    <a href="<?=base_url('masters/degree')?>" class="btn btn-sm btn-default" title="Reset Filters" style="border-radius: 6px; height: 34px; line-height: 22px; white-space: nowrap;">
                ...
                <table class="table table-hover table-striped" style="margin: 0;">
                  <thead>
                    <tr>
                      ...
                      <th>Degree / Qualification Name</th>
                    ...
                    <?php else: ?>
                      <tr>
                        <td colspan="4" style="text-align: center; padding: 40px 20px; color: #94a3b8;">
                          ...
                          <p style="font-size: 14px; font-weight: 500; margin: 0;">No degree records found.</p>
                        </td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
    ...
    <div class="modal-content" style="border-radius: 12px; border: none; box-shadow: 0 10px 25px rgba(0,0,0,0.15); overflow: hidden;">
      <div class="modal-body" style="padding: 28px 24px; text-align: center;">
        ...
        <p style="font-size: 13.5px; color: #64748b; margin-bottom: 20px;">
          Are you sure you want to delete <strong id="delete-item-title" style="color: #1e293b;">this degree</strong>? This action cannot be undone.
        </p>
...
$(document).ready(function(){
  ...
  $('.select-edit-btn').click(function(){
    ...
    $('#eid').val(id);
    $('#degree_name').val(name).focus();
    $('#form-title').text('Edit Degree');
    ...
    $('#submit-text').text('Update Degree');
  ...
  $('.status-toggle-btn').click(function(){
    ...
    var uri = '<?=base_url('masters/degree/statusupdate')?>';
  ...
  $('#confirm-delete-btn').click(function(){
    if(!deleteId) return;
    var uri = '<?=base_url('masters/degree/delete')?>/' + deleteId;
