...
<br>  
  ...
  <div class="row text">
	...
	<form action="<?=base_url()?>masters/education/create" method="post" id="myform">
	...
	<div style="border:1px solid #d2cbcb;padding:25px; border-bottom-left-radius: 4px;; border-bottom-right-radius: 4px;">
		<div class="form-group">
		  ...
		  <input type="text" class="form-control" id="education" placeholder="Enter education name" name="education" data-validation="required"
    ...
    <thead>
      <tr>
        <th class="tableheaddata">Education ID</th>
        <th class="tableheaddata">Education Name</th>
    ...
    <tbody>
	<?php 
		$getlists=$this->db->get_where('master_education');
	...
	?>
      <tr class="active">
        <td class="tabledata"><?=$rowdata->education_id;?></td>
        <td class="tabledata"><?=$rowdata->education_name;?></td>
		
        <td class="tabledata"><a href="#" class="statuscng " id="ch<?=$rowdata->education_id;?>" data-uid="<?=$rowdata->education_id;?>" style="cursor:pointer"><span class="<?=$statusclass?>" ><?=$statusvalue?></span></a></td>
        <td class="tabledata" id="kamal"><a href="#" style="cursor:pointer" class="select" data-uid="<?=base64_encode($rowdata->education_id)?>" data-name="<?=$rowdata->education_name;?>">Select</a></td>
		
        <td class="tabledata"><a href="#" style="cursor:pointer" class="delete" data-uid="<?=$rowdata->education_id;?>">Delete</a></td>
      </tr>
		<?php } ?>
    </tbody>
  </table>
...
$(document).ready(function(){
    ...
    $(".delete").click(function(){
		...
		if(c)
		{
			...
			var uri='<?=base_url()?>masters/education/delete/'+uid
			$.ajax({
			 type:"post", 
			 url: uri,
...
$(document).ready(function(){
	$(".statuscng").click(function(){
		...
		var uri='<?=base_url()?>masters/education/statusupdate';
			$.ajax({
			 type:"post", 
			 url: uri,
			 data:{uid:uid},
