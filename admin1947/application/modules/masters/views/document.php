...
<br>  
  ...
  <div class="row text">
	...
	<form action="<?=base_url()?>masters/document/create" method="post" id="myform">
	<div class="formheader">
		ID Document Master
	</div>
	<div class="formdiv">
		<div class="form-group">
		  <label for="text">ID Document Name:</label>
		  <input type="text" class="form-control" id="document" placeholder="Enter document name" name="document" data-validation="required"
    ...
    <thead>
      <tr>
        <th class="tableheaddata">Document ID</th>
        <th class="tableheaddata">Document Name</th>
    ...
    <tbody>
	<?php 
		$getlists=$this->db->get_where('master_document');
	...
	?>
      <tr class="active">
        <td class="tabledata"><?=$rowdata->document_id;?></td>
        <td class="tabledata"><?=$rowdata->document_name;?></td>
		
        <td class="tabledata"><a href="#" class="statuscng " id="ch<?=$rowdata->document_id;?>" data-uid="<?=$rowdata->document_id;?>" style="cursor:pointer"><span class="<?=$statusclass?>" ><?=$statusvalue?></span></a></td>
        <td class="tabledata" id="kamal"><a href="#" style="cursor:pointer" class="select" data-uid="<?=base64_encode($rowdata->document_id)?>" data-name="<?=$rowdata->document_name;?>">Select</a></td>
		
        <td class="tabledata"><a href="#" style="cursor:pointer" class="delete" data-uid="<?=$rowdata->document_id;?>">Delete</a></td>
      </tr>
		<?php } ?>
    </tbody>
  </table>
...
$(document).ready(function(){
    ...
    $(".delete").click(function(){
		...
		if(c)
		{
			...
			var uri='<?=base_url()?>masters/document/delete/'+uid
			$.ajax({
			 type:"post", 
			 url: uri,
...
$(document).ready(function(){
	$(".statuscng").click(function(){
		...
		var uri='<?=base_url()?>masters/document/statusupdate';
			$.ajax({
			 type:"post", 
			 url: uri,
			 data:{uid:uid},
