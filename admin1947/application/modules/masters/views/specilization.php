<div class="content-wrapper">
  ...
  <section class="content-header" style="padding: 20px 20px 10px;">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
      <div>
        <h1 style="font-size: 22px; font-weight: 700; color: #1e293b; margin: 0;">Specialization Master</h1>
        <small style="color: #64748b; font-size: 13px;">Manage medical specializations and department icons</small>
      </div>
      <ol class="breadcrumb" style="position: static; float: none; margin: 0; background: transparent; padding: 0;">
        ...
        <li><a href="<?=base_url('masters/specilization')?>" style="color: #64748b;">Masters</a></li>
        <li class="active" style="color: #1e293b; font-weight: 600;">Specialization</li>
      </ol>
    </div>
  </section>
      ...
      <div class="row">
          ...
          <div class="master-card master-sticky-form">
            <div class="master-card-header">
              <h3 class="master-card-title">
                ...
                <span id="form-title">Add Specialization</span>
              </h3>
            ...
            <form action="<?=base_url('masters/specilization/create')?>" method="post" id="master-form" enctype="multipart/form-data" class="master-card-body">
              <input type="hidden" id="eid" name="eid" value="">

              <div class="form-group" style="margin-bottom: 18px;">
                <label for="specilization_name" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px;">
                  Specialization Name <span style="color: #ef4444;">*</span>
                </label>
                <input type="text" class="form-control" id="specilization_name" name="specilization" 
                       placeholder="e.g. Cardiology, Neurology" required
                       style="border-radius: 8px; height: 40px;">
                <small class="text-muted" style="font-size: 11.5px; margin-top: 4px; display: block;">Enter a unique medical specialization title.</small>
              </div>

              <!-- Upload Icon Dropzone -->
              <div class="form-group" style="margin-bottom: 20px;">
                <label style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px;">Specialization Icon (Optional)</label>
                <div class="upload-dropzone" onclick="document.getElementById('uploadimage').click();">
                  <i class="fa fa-cloud-upload fa-2x" style="color: #00a896; margin-bottom: 4px;"></i>
                  <p style="margin: 0; font-size: 13px; font-weight: 500; color: #475569;">Click to upload image icon</p>
                  <span style="font-size: 11px; color: #94a3b8;">PNG, JPG, JPEG up to 2MB</span>
                  <div>
                    <img id="image_preview" class="upload-preview-thumb" src="#" alt="Preview">
                  </div>
                </div>
                <input type="file" id="uploadimage" name="image" accept="image/*" style="display: none;" onchange="previewImage(this);">
              </div>
              ...
              <div style="display: flex; gap: 10px; justify-content: flex-end; align-items: center; border-top: 1px solid #e2e8f0; padding-top: 16px;">
                ...
                <button type="submit" id="submit-btn" name="submit" class="btn btn-primary" style="border-radius: 8px; font-weight: 600; padding: 8px 22px; background: #00a896; border-color: #00a896;">
                  <i class="fa fa-check"></i> <span id="submit-text">Add Specialization</span>
                </button>
              </div>
            </form>
          </div>
        ...
        <div class="col-lg-8 col-md-7">
          <div class="master-card">
            <div class="master-card-header">
              <h3 class="master-card-title">
                <i class="fa fa-list" style="color: #00a896;"></i>
                <span>Specialization List</span>
              </h3>
              <span style="font-size: 12px; color: #64748b; font-weight: 500;">
                Total Records: <strong><?=(isset($config['total_rows']) ? $config['total_rows'] : count($specilization));?></strong>
              </span>
            </div>

            <div class="master-card-body" style="padding: 16px 20px;">
              <!-- Search & Records Per Page Toolbar -->
              <form action="<?=base_url('masters/specilization')?>" method="get" id="search_form" class="master-toolbar">
                ...
                <div style="display: flex; align-items: center; gap: 8px; flex-grow: 1; max-width: 380px;">
                  <div class="input-group input-group-sm" style="width: 100%;">
                    <input type="text" class="form-control" name="keyword" placeholder="Search by specialization name..." 
                  ...
                  <?php if($this->input->get_post('keyword')!=''): ?>
                    <a href="<?=base_url('masters/specilization')?>" class="btn btn-sm btn-default" title="Clear Search" style="border-radius: 6px; height: 34px; line-height: 22px;">
              ...
              <div class="table-responsive" style="border-radius: 8px; border: 1px solid #e2e8f0;">
                <table class="table table-hover table-striped" style="margin: 0;">
                  ...
                  <tbody>
                    <?php if(!empty($specilization)): ?>
                      <?php foreach($specilization as $rowdata): 
                        $status = $rowdata['status'];
                        $isActive = ($status == 1);
                        $iconUrl = !empty($rowdata['image_icon']) ? base_url('public/assets/upload/'.$rowdata['image_icon']) : '';
                      ?>
                        <tr id="row-<?=$rowdata['id'];?>">
                          <td style="text-align: center; font-weight: 600; color: #64748b;"><?=$rowdata['id'];?></td>
                          <td style="text-align: center;">
                            <?php if(!empty($rowdata['image_icon'])): ?>
                              <img src="<?=$iconUrl;?>" alt="Icon" style="width: 36px; height: 36px; border-radius: 6px; object-fit: cover; border: 1px solid #e2e8f0;">
                            <?php else: ?>
                              <div style="width: 36px; height: 36px; border-radius: 6px; background: #f1f5f9; display: inline-flex; align-items: center; justify-content: center; color: #94a3b8;">
                    ...
                    <?php else: ?>
                      <tr>
                        <td colspan="5" style="text-align: center; padding: 40px 20px; color: #94a3b8;">
                          ...
                          <p style="font-size: 14px; font-weight: 500; margin: 0;">No specialization records found.</p>
                        </td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
    ...
    <div class="modal-content" style="border-radius: 12px; border: none; box-shadow: 0 10px 25px rgba(0,0,0,0.15); overflow: hidden;">
      <div class="modal-body" style="padding: 28px 24px; text-align: center;">
        ...
        <p style="font-size: 13.5px; color: #64748b; margin-bottom: 20px;">
          Are you sure you want to delete <strong id="delete-item-title" style="color: #1e293b;">this item</strong>? This action cannot be undone.
        </p>
...
$(document).ready(function(){
  ...
  $('.select-edit-btn').click(function(){
    ...
    $('#eid').val(id);
    $('#specilization_name').val(name).focus();
    $('#form-title').text('Edit Specialization');
    ...
    $('#submit-text').text('Update Specialization');

    if(icon && icon !== '') {
      $('#image_preview').attr('src', icon).show();
    } else {
      $('#image_preview').hide();
    }
  ...
  $('#reset-btn').click(function(){
    ...
    $('#form-title').text('Add Specialization');
    ...
    $('#submit-text').text('Add Specialization');
  });
  ...
  $('.status-toggle-btn').click(function(){
    ...
    var uri = '<?=base_url('masters/specilization/statusupdate')?>';
  ...
  $('#confirm-delete-btn').click(function(){
    if(!deleteId) return;
    var uri = '<?=base_url('masters/specilization/delete')?>/' + deleteId;
