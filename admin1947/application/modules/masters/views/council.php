<div class="content-wrapper">
  ...
  <section class="content-header" style="padding: 20px 20px 10px;">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
      <div>
        <h1 style="font-size: 22px; font-weight: 700; color: #1e293b; margin: 0;">Medical Council Master</h1>
        <small style="color: #64748b; font-size: 13px;">Manage medical councils and state registration boards</small>
      </div>
      <ol class="breadcrumb" style="position: static; float: none; margin: 0; background: transparent; padding: 0;">
        ...
        <li><a href="<?=base_url('masters/council')?>" style="color: #64748b;">Masters</a></li>
        <li class="active" style="color: #1e293b; font-weight: 600;">Council</li>
      </ol>
    </div>
  </section>
          ...
          <div class="master-card master-sticky-form">
            <div class="master-card-header">
              <h3 class="master-card-title">
                ...
                <span id="form-title">Add Council</span>
              </h3>
            ...
            <form action="<?=base_url('masters/council/create')?>" method="post" id="master-form" class="master-card-body">
              <input type="hidden" id="eid" name="eid" value="">

              <div class="form-group" style="margin-bottom: 20px;">
                <label for="council_name" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px;">
                  Council Name <span style="color: #ef4444;">*</span>
                </label>
                <input type="text" class="form-control" id="council_name" name="specilization" 
                       placeholder="e.g. Medical Council of India, Uttar Pradesh Medical Council" required
                       style="border-radius: 8px; height: 40px;">
                <small class="text-muted" style="font-size: 11.5px; margin-top: 4px; display: block;">Enter official medical council or board name.</small>
              </div>
              ...
              <div style="display: flex; gap: 10px; justify-content: flex-end; align-items: center; border-top: 1px solid #e2e8f0; padding-top: 16px;">
                ...
                <button type="submit" id="submit-btn" name="submit" class="btn btn-primary" style="border-radius: 8px; font-weight: 600; padding: 8px 22px; background: #00a896; border-color: #00a896;">
                  <i class="fa fa-check"></i> <span id="submit-text">Add Council</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div class="master-card">
            <div class="master-card-header">
              <h3 class="master-card-title">
                ...
                <span>Medical Council List</span>
              </h3>
              <span style="font-size: 12px; color: #64748b; font-weight: 500;">
                Total Records: <strong><?=(isset($total_rows) ? $total_rows : (isset($config['total_rows']) ? $config['total_rows'] : (is_array($council) ? count($council) : 0)));?></strong>
              </span>
            </div>

            <div class="master-card-body" style="padding: 16px 20px;">
              <!-- Search & Filters Toolbar -->
              <form action="<?=base_url('masters/council')?>" method="get" id="search_form" class="master-toolbar" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                ...
                <div style="display: flex; align-items: center; gap: 8px; flex-grow: 1; max-width: 380px; justify-content: flex-end;">
                  <div class="input-group input-group-sm" style="width: 100%;">
                    <input type="text" class="form-control" name="keyword" placeholder="Search by council name..." 
                  ...
                  <?php if($this->input->get('keyword')!='' || ($this->input->get('status')!==null && $this->input->get('status')!='')): ?>
                    <a href="<?=base_url('masters/council')?>" class="btn btn-sm btn-default" title="Reset Filters" style="border-radius: 6px; height: 34px; line-height: 22px; white-space: nowrap;">
                ...
                <table class="table table-hover table-striped" style="margin: 0;">
                  ...
                  <tbody>
                    <?php if(!empty($council)): ?>
                      <?php foreach($council as $rowdata): 
                    ...
                    <?php else: ?>
                      <tr>
                        <td colspan="4" style="text-align: center; padding: 40px 20px; color: #94a3b8;">
                          ...
                          <p style="font-size: 14px; font-weight: 500; margin: 0;">No council records found.</p>
                        </td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
    ...
    <div class="modal-content" style="border-radius: 12px; border: none; box-shadow: 0 10px 25px rgba(0,0,0,0.15); overflow: hidden;">
      <div class="modal-body" style="padding: 28px 24px; text-align: center;">
        ...
        <p style="font-size: 13.5px; color: #64748b; margin-bottom: 20px;">
          Are you sure you want to delete <strong id="delete-item-title" style="color: #1e293b;">this council</strong>? This action cannot be undone.
        </p>
...
$(document).ready(function(){
  ...
  $('.select-edit-btn').click(function(){
    ...
    $('#eid').val(id);
    $('#council_name').val(name).focus();
    $('#form-title').text('Edit Council');
    ...
    $('#submit-text').text('Update Council');
  ...
  $('.status-toggle-btn').click(function(){
    ...
    var uri = '<?=base_url('masters/council/statusupdate')?>';
  ...
  $('#confirm-delete-btn').click(function(){
    if(!deleteId) return;
    var uri = '<?=base_url('masters/council/delete')?>/' + deleteId;
