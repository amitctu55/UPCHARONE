...
<br>  
  ...
  <div class="row text">
	...
	<form action="<?=base_url()?>masters/course/create" method="post" id="myform">
	...
	<div class="formdiv">
		<div class="form-group">
		  ...
		  <input type="text" class="form-control" id="course" placeholder="Enter course name" name="course" data-validation="required"
		...
		<div class="row">
			<div class="form-group col-md-6">
			  ...
			  <input type="text" class="form-control" id="coursecode" placeholder="Enter course code" name="coursecode" data-validation="required"
				data-validation-error-msg="This Field is required">
			</div>
			<div class="form-group col-md-6">
    ...
    <thead>
      <tr>
        <th class="tableheaddata">Course ID</th>
        <th class="tableheaddata">Course Name</th>
		<th class="tableheaddata">Course Code</th>
    ...
    <tbody>
	<?php 
		$getlists=$this->db->get_where('master_course');
	...
	?>
      <tr class="active">
        <td class="tabledata"><?=$rowdata->course_id;?></td>
        <td class="tabledata"><?=$rowdata->course_name;?></td>
		<td class="tabledata"><?=$rowdata->course_code;?></td>
		
        <td class="tabledata"><a href="#" class="statuscng " id="ch<?=$rowdata->course_id;?>" data-uid="<?=$rowdata->course_id;?>" style="cursor:pointer"><span class="<?=$statusclass?>" ><?=$statusvalue?></span></a></td>
        <td class="tabledata" id="kamal"><a href="#" style="cursor:pointer" class="select" data-uid="<?=base64_encode($rowdata->course_id)?>" data-name="<?=$rowdata->course_name;?>" data-code="<?=$rowdata->course_code;?>" data-status="<?=$rowdata->status;?>" >Select</a></td>
		
        <td class="tabledata"><a href="#" style="cursor:pointer" class="delete" data-uid="<?=$rowdata->course_id;?>">Delete</a></td>
      </tr>
		<?php } ?>
    </tbody>
  </table>
...
$(document).ready(function(){
    ...
    $(".delete").click(function(){
		...
		if(c)
		{
			...
			var uri='<?=base_url()?>masters/course/delete/'+uid
			$.ajax({
			 type:"post", 
			 url: uri,
...
$(document).ready(function(){
	$(".statuscng").click(function(){
		...
		var uri='<?=base_url()?>masters/course/statusupdate';
			$.ajax({
			 type:"post", 
			 url: uri,
			 data:{uid:uid},
