      ...
      <div class="content-wrapper">
        <section class="content">
          <div class="container bg-3">  
            <div class="row text-">
              <form class="form-horizontal formbody" id='mainform' action="<?=base_url()?>doctor/appointment/account" method="get" enctype="multipart/form-data">
                   ...
                   <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-3">
                        ...
                        <?php 
                         if( $this->input->get_post('hospital_name')!='' ||  $this->input->get_post('doctor_name')!='' ||  $this->input->get_post('payment_mode')!='' ||  $this->input->get_post('date_from')!='' ||  $this->input->get_post('date_to')!='' ||  $this->input->get_post('session_from')!='' ||  $this->input->get_post('session_to')!=''  )
                          { 
                            echo anchor("doctor/appointment/account/",'<span>Clear Search</span>');    
                          } 
                        ?>
                      </div>
                    </div>
              ...
              <h4 style="font-weight:600;margin-bottom:20px;"></h4>
              ...
              <table class="table table-hover table-bordered table-bordered" id='example' style="border:none;">
                <thead>
                  <tr>
                    ...
                    <th class="tableheaddata">Collection</th>
                    <th class="tableheaddata">Fee Status</th>
                    <th class="tableheaddata">Details</th>
                  </tr>
                </thead>
                <tbody id="tviewtablebody">
                  <?php $i=1;
                  foreach($hospital_account as $hospital)
                  {
                  echo"<tr>";
                  echo"<td>".$i."</td>";
                  echo"<td>".$hospital->hospital_name."</td>";
                  echo"<td>".$hospital->city_name."</td>";
                  echo"<td> Rs. ".$hospital->total."</td>";
                  if($hospital->payment_status=='DONE'){ echo"<td> Received </td>"; }
                  echo "<td>"; ?>
                  <a href="<?php echo base_url(); ?>doctor/appointment/account_appointment?hospital_name=<?php if($this->input->get('hospital_name')!=''){ echo $this->input->get('hospital_name');} else{ echo $hospital->institute_id; } ?>&doctor_name=<?php echo $this->input->get('doctor_name')?>&payment_mode=<?php echo $this->input->get('payment_mode')?>&date_from=<?php echo $this->input->get('date_from')?>&date_to=<?php echo $this->input->get('date_to')?>" style=color:red;>View Appointment</a>
                  <?php echo  "</td>";
                  echo"</tr>";
                  $i++;}
                  ?>
          ...
          <section class="content">
            <div class="container bg-3">  
            <div class="row text-">
              <h3 style="font-weight:600;margin-bottom:20px;">Monthly Income Overview</h3>
              <div class="col-md-6">
                <div class="formbody">
                  <div class="row mainheadlinefirstrow">
                    <div class="col-md-12 mainheadlinefirst">Monthly Income</div>
                  </div>
                  <table class="table table-bordered" id='mydata' style="border:none;">
                    <thead>
                      <tr>
                        <th class="tableheaddata">Month</th>
                        <th class="tableheaddata">Online Payment</th>
                        <th class="tableheaddata">Cash Payment</th>
                        <th class="tableheaddata">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                    <?php 
                    ...
                    <?php for($i=$month; $i>0; $i--){?>
                      <tr class="active"> 
                        ...
                        ?></td>
                        <td class="tabledata"><?php  $online = $this->appointmentmodel->get_monthly_totals($year,$i,'ONLINE'); 
                        if($online[0]['TotalAmount']!=''){ echo $online[0]['TotalAmount']; } else{ echo '0';}
                        ?></td>
                        <td class="tabledata"><?php  $cash_on_counter = $this->appointmentmodel->get_monthly_totals($year,$i,'COC'); 
                        if($cash_on_counter[0]['TotalAmount']!=''){ echo $cash_on_counter[0]['TotalAmount']; } else{ echo '0';}
                        ?></td>
                        <td class="tabledata"><?php  $total = $this->appointmentmodel->get_monthly_totals($year,$i);
                        if($total[0]['TotalAmount']!=''){ echo $total[0]['TotalAmount']; } else{ echo '0';}
                        ?></td>
                      </tr>
                    <?php } ?>
                    </tbody>
