...
{ 
	...
	{		
		...
		$result = $this->db->get_where('appointment', array('appointment_id'))->result_array();
		return $result;
	}
	...
	{		
		...
		$result = $this->db->get_where('hospital', array('uid'))->result();
		return $result;
	}
	...
	{		 
		...
		if($hospital_name!='')
		{
			$this->db->where("(appointment.institute_id LIKE '%".$hospital_name."%' )");
		}
		if($doctor_name!='')
		{
			$this->db->where("appointment.doctor_id","$doctor_name");
			//x$this->db->where("(appointment.institute_id LIKE '%".$hospital_name."%' )");
		}
		if($payment_mode!='')
		{
		...
		if($date_from!='')
		{
			$this->db->where('appointment.pay_date >=', $date_from);
		}
		if($date_to!='')
		{
			$this->db->where('appointment.pay_date <=', $date_to);
		}
		if($session_from!='')
		{	
			$this->db->where('appointment.pay_date >=', $session_from);
		}
		if($session_to!='')
		{	
			$this->db->where('appointment.pay_date <=', $session_to);
		}
		else
		{ 
		  $this->db->order_by('appointment_id','asc');
		}
		$this->db->where("appointment.payment_status","DONE");
		//$this->db->select('appointment.*');
		$this->db->select('institute_id, COUNT(institute_id) as count ,SUM(amount) as total,payment_status,hospital.name as hospital_name,master_city.name as city_name');
		$this->db->group_by("institute_id");
		$this->db->join('hospital','hospital.id = appointment.institute_id');
	...
	{		 
		...
		$this->db->select('appointment.* ,hospital.name as hospital_name');
		$this->db->join('hospital','hospital.id = appointment.institute_id');
		$this->db->join('master_city','master_city.id = hospital.city','left');
		$result = $this->db->get('appointment')->result();
		return $result;
	...
	{		
		...
		$this->db->select('profile_dr.*');
		$result = $this->db->get_where('profile_dr', array('uid'))->result();
		return $result;
	}
	...
	public function get_appointment_by_droctor_id()
	{		
		...
		if($session_from!='')
		{	
			$this->db->where('appointment_date >=', $session_from);
		}
		if($session_to!='')
		{	
			$this->db->where('appointment_date <=', $session_to);
		}
		if($doctor_id!='')
		{
	...
	{	
		$this->db->select("sum(amount) as TotalAmount", false);
		...
		if ($theYear != '') 
		{	
			$this->db->where("EXTRACT(Year FROM pay_date) = '" . $theYear . "'", NULL, FALSE);
		}
		if ($theMonth != '')
		{	
			$this->db->where("EXTRACT(Month FROM pay_date) = '" . $theMonth . "'", NULL, FALSE);
		}
